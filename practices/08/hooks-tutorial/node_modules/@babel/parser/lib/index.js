...
class StatementParser extends ExpressionParser {
  ...
  isLet(declaration) {
    ...
    if (nextCh === 123 && !lineBreak.test(this.state.input.slice(this.state.end, next)) || nextCh === 91) {
      return true;
    }

    if (isIdentifierStart(nextCh)) {
      if (!declaration && lineBreak.test(this.state.input.slice(this.state.end, next))) {
        return false;
      }
